...
export function useDevices(
  ...
  options?: Omit<
    UseQueryOptions<DevicePaginatedResponse>,
    "queryKey" | "queryFn"
  >,
) {
  const queryFilters = {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<DevicePaginatedResponse> => {
      const { data } = await apiClient.get(ENDPOINTS.DEVICES.LIST, {
        params: queryFilters,
      });
      return data;
    },
...
export function useUpdateDevice() {
  ...
  return useMutation({
    mutationFn: async ({
      id,
      ...deviceData
    }: UpdateDeviceRequest): Promise<DeviceApiResponse> => {
      const { data } = await apiClient.put(
        ENDPOINTS.DEVICES.UPDATE(id),
        deviceData,
      );
      return data;
    },
