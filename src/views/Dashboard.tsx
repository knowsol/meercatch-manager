...
export default function Dashboard() {
  ...
  // 오늘 날짜와 이번 주 월요일 계산
  const { todayStr, mondayStr, weekRangeStr } = useMemo(() => {
    const now = new Date();
    ...
    return {
      ...
      weekRangeStr: `${formatShortDate(mondayDate)} ~ ${formatShortDate(todayDate)}`,
    };
  }, []);
  ...
  // 최근 탐지 (전체)
  const { data: detectionData } = useDetectionSearch({
    page: 0,
    size: 5,
    eventType: "",
    osType: "",
  ...
  // 오늘 탐지
  const { data: todayDetectionData } = useDetectionSearch({
    page: 0,
    size: 1,
    eventType: "",
    osType: "",
  ...
  // 이번 주 탐지 (월요일 ~ 오늘)
  const { data: weekDetectionData } = useDetectionSearch({
    page: 0,
    size: 1,
    eventType: "",
    osType: "",
  ...
  const licUsePct =
    totalLicenseCount > 0
      ? Math.round((usedLicenseCount / totalLicenseCount) * 100)
      : 0;

  const recentDetections = detectionData?.data ?? [];

  // 오늘 탐지 통계
  const todayTotalDetections = todayDetectionData?.meta?.totalCount ?? 0;

  // 이번 주 탐지 통계
  const weekTotalDetections = weekDetectionData?.meta?.totalCount ?? 0;
  ...
  const detCols = [
    ...
    },
    {
      ...
      width: "15%",
      render: (v: number) => (
        <DetTypeBadge type={EVENT_TYPE_MAP[v] || "기타"} />
      ),
    },
    {
      key: "deviceUuid",
    ...
    },
    {
      ...
      label: "OS",
      width: "30%",
      render: (v: number) => DETECT_OS_TYPE_MAP[v] || `OS ${v}`,
    },
  ];
  ...
  const pauseCols = [
    {
      ...
      render: (v) => {
        ...
        const sch = grp
          ? DUMMY.schools.find((sc) => sc.schoolId === grp.schoolId)
          : null;
        return sch ? sch.name : "—";
      },
    },
  ...
  return (
    <div>
      ...
      <div className="grid-4 section-gap">
        ...
        <KPI
          label="활성 라이선스"
          value={usedLicenseCount}
          sub="현재 사용 중"
          color="ok"
        />
        <KPI
          label="오늘 탐지"
          value={todayTotalDetections}
          sub={todayStr}
          color="err"
        />
        <KPI
          label="이번 주 탐지"
          value={weekTotalDetections}
          sub={weekRangeStr}
          color="warn"
        />
      </div>
      ...
      {!isDirect && (
        <div className="grid-4 section-gap">
          ...
          <KPI
            ...
            sub={`온라인 ${s.onlineDevices}대`}
            color="ok"
          />
          <KPI
            label="활성 중단"
            value={s.activePauses}
            sub="현재 탐지 중단 중"
            color="warn"
          />
        </div>
      ...
      <div className="card section-gap">
        ...
        <Table
          ...
          onRowClick={(row: Detection) =>
            openPanel(<DetectionDetailPanel detection={row} />)
          }
        />
      </div>
      ...
      {isDirect && (
        <div className="card mt-20">
          ...
          <dl className="info-row">
            {/* <dt>라이선스 유형</dt>{" "}
            <dd>
              {licenseData?.data?.[0]?.detectType === "11"
                ? "All-in-One"
                : licenseData?.data?.[0]?.detectType === "10"
                  ? "선정성"
                  : "도박"}
            </dd> */}
            <dt>OS 종류</dt>{" "}
            <dd>
              {/* {licenseData?.meta?.totalCount ?? 0}종 */}
              Windows, Android, iOS, ChromeBook, WhaleBook
            </dd>
            <dt>단말 사용</dt>{" "}
            <dd>
              {usedLicenseCount} / {totalLicenseCount}대 ({licUsePct}%)
            </dd>
          </dl>
          <div className="mt-16">
