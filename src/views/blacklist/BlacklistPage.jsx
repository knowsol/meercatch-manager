...
function KebabMenu({ onDelete, onEdit, toast }) {
  ...
  return (
    <div ref={ref} style={{ position:'relative', display:'inline-block' }}>
      ...
      {open && (
        <div style={{ position:'absolute', right:0, top:'calc(100% + 4px)', background:'var(--bg1)', border:'1px solid var(--bd)', borderRadius:6, boxShadow:'0 4px 16px rgba(0,0,0,.12)', zIndex:500, minWidth:120, overflow:'hidden' }}>
...
export default function BlacklistPage() {
  ...
  const actionBar = selected.length > 0 && (
    <div style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 12px', background:'rgba(239,68,68,0.06)', borderRadius:6, marginBottom:8, fontSize:13 }}>
      <span style={{ color:'#ef4444', fontWeight:600 }}>{selected.length}개 선택됨</span>
      <button className="btn btn-outline" style={{ fontSize:12, height:28, padding:'0 10px', color:'#ef4444', borderColor:'#ef4444' }}
