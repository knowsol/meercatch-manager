...
const SAMPLE_COLS = [
  { key: 'name', label: '이름' },
  { key: 'role', label: '역할', width: '90px', render: (v: string) => <Badge cls={v === 'admin' ? 'bdg-ac' : v === 'staff' ? 'bdg-ok' : 'bdg-muted'}>{v}</Badge> },
  { key: 'status', label: '상태', width: '80px', render: (v: string) => <StatusBadge status={v} /> },
