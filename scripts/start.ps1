# start.ps1 — Start all services for reservation
...
if (-not (Test-Path "$WorkspaceRoot\.env")) {
    ...
    Write-Host "       Re-run: .\shared\scripts\create-workspace.ps1 reservation" -ForegroundColor DarkGray
    exit 1
}
...
# ── Start ─────────────────────────────────────────────────────────────────────
Write-Host "Starting reservation (web=$webPort api=$apiPort admin=$adminPort)..." -ForegroundColor Cyan
docker compose up -d

Write-Host ""
Write-Host "  reservation services running:" -ForegroundColor Green
